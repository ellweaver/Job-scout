...
def manual_search():
    ...
    search_directory = input(
        "Please enter the directory for your search file [./search_queries/]: "
    ).strip()
    search_filename = input(
        "Please enter the filename of your search file [default_search.json]: "
    ).strip()
    destination_directory = input(
        "Please enter an existing save location for your query [./search_results/]: "
    ).strip()
...
def generate_search_file(
    ...
    if advanced == True:
        ...
        user_response = (
            input("work from home [False]: ").strip().lower().startswith("t")
        )
        if user_response:
    ...
    not_none_params = {k: v for k, v in params.items() if v != "" and v != []}

    event = {"api_key": {}, "url": url, "params": not_none_params}

    while not file_name:
        user_response = input(
            f"Please enter your Search file name without extension[{query}]: "
        ).lstrip()
        if user_response:
    ...
    return {
        ...
        "search_directory": search_directory,
        "filename": file_name,
        "filepath": search_filepath,
    }


def list_search_directory(search_directory="search_queries/"):
    """
    lists all files in specified directory returns file path chosen by
    user input check file has url and params keys and query key in params
    """
